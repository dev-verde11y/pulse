...
export function EditHeroBannerModal({ banner, isOpen, onClose, onSuccess }: EditHeroBannerModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || 'Erro ao atualizar hero banner')
      }

      onSuccess()
    } catch (error) {
      console.error('Error updating hero banner:', error)
      alert(error instanceof Error ? error.message : 'Erro ao atualizar hero banner')
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 border border-gray-700 rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="p-6 border-b border-gray-700/50">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-black text-white">Editar Hero Banner</h2>
              <p className="text-sm text-gray-400 mt-1">
                Editando: {banner.title}
              </p>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="flex-1 p-6 overflow-y-auto">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              {formData.backgroundImage && (
                <div className="mt-4">
                  ...
                  <div className="w-full max-w-md h-40 bg-gray-700 rounded-lg overflow-hidden">
                    <img 
            ...
            <div className="bg-gradient-to-br from-orange-600/10 via-orange-700/5 to-orange-800/5 border border-orange-500/20 rounded-xl p-4">
              <h4 className="text-sm font-medium text-orange-400 mb-3">Informações Atuais</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
                <div>
                  <p className="text-gray-400">ID:</p>
                  <p className="text-gray-300 font-mono">{banner.id.slice(0, 8)}...</p>
                </div>
                <div>
            ...
            {selectedAnime && (
              <div className="bg-gradient-to-br from-blue-600/10 via-blue-700/5 to-blue-800/5 border border-blue-500/20 rounded-xl p-4">
                <h4 className="text-sm font-medium text-blue-400 mb-3">Anime Selecionado</h4>
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-blue-400 text-sm font-mono">ID: {selectedAnime.id.slice(0, 8)}...</p>
                  </div>
                </div>
              </div>
            )}
