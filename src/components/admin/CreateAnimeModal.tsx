  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || 'Erro ao criar anime')
      }
      ...
      alert(error instanceof Error ? error.message : 'Erro ao criar anime')
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 border border-gray-700 rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden">
        ...
        <div className="p-6 border-b border-gray-700/50">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-black text-white">Criar Novo Anime</h2>
              <p className="text-sm text-gray-400 mt-1">Adicione um novo anime ao catálogo</p>
            </div>
            <button
              onClick={onClose}
              disabled={loading}
        ...
        <div className="p-6 border-t border-gray-700/50 flex items-center justify-end space-x-4">
          ...
          >
            ...
            <span>{loading ? 'Criando...' : 'Criar Anime'}</span>
          </button>
        </div>
      </div>
    </div>
